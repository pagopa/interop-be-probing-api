...
class EserviceTest {
    ...
    void testEserviceEntity_whenDefaultValuesAreSet_returnsEservice(){
        ...
        assertEquals(OffsetDateTime.of(OffsetDateTime.now().getYear(), OffsetDateTime.now().getMonthValue(),
                        OffsetDateTime.now().getDayOfMonth(), 23, 59, 0, 0, ZoneOffset.UTC),
                eserviceDuplicate.getPollingEndTime(), "Values should be equal");
        assertEquals(OffsetDateTime.of(OffsetDateTime.now().getYear(), OffsetDateTime.now().getMonthValue(),
                        OffsetDateTime.now().getDayOfMonth(), 0, 0, 0, 0, ZoneOffset.UTC),
    ...
    void testEserviceEntity_whenDefaultValuesAreCorrectlyOverwritten_returnsEservice(){
        ...
        assertEquals(OffsetDateTime.of(2023, 12, 12, 1, 0, 0,
                0, ZoneOffset.UTC), eserviceDuplicate.getPollingEndTime(), "Values should be equal");
        assertEquals(OffsetDateTime.of(2023, 12, 12, 1, 0, 0,
                0, ZoneOffset.UTC), eserviceDuplicate.getPollingStartTime(), "Values should be equal");
    ...
    void testEserviceEntity_whenBasePathIsTooLong_throwsException(){
        eservice.setBasePath(new String[]{RandomStringUtils.randomAlphabetic(256)});
